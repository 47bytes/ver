...
GO               = go
GOFLAGS         ?= $(GOFLAGS:)
GOLINKERFLAGS ?= -ldflags "-X main.BUILD_VERSION=${VERSION} -X main.BUILD_HASH=${HASH} -X main.BUILD_DATE=${DATE}"
GODOC            = godoc
TIMEOUT          = 15
...
.PHONY: build 
build: fmt lint $(BIN) ; $(info $(M) building binary...) @ ## Build program binary
	$Q $(GO) build \
		$(GOFLAGS) \
		-tags release \
		$(GOLINKERFLAGS) \
